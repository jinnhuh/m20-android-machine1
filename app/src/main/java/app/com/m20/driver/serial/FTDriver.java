...
public class FTDriver {
    ...
    private static final String TAG = "M20_FTDriver";
	...
	public boolean begin(int baudrate) {
        for (UsbDevice device : mManager.getDeviceList().values()) {
            Log.d(TAG, String.format(Locale.US, "Devices : %s.", device.toString()));
        ...
        Log.i(TAG, String.format(Locale.US, "Device Serial : %s.", mDeviceConnection.getSerial()));
        return true;
    }
...
//        }
            ...
            if (!mReadPakcetChecker) {
                ...
                for (int i = 0; i < buf.length; i++) {
                    ...
                    while ((incReadCount - 1) % 10 != (Byte.valueOf(buf[i]) - '0')) {
                        Log.d(TAG, "!!! Lost Data !!! count : " + (incReadCount - 1) + ", data : " + buf[i]);
                        ++incReadCount;
                    }
                }
        ...
        try {
            len = mDeviceConnection.bulkTransfer(mFTDIEndpointIN[channel], mReadbuf, mReadbuf.length, 0); // RX
        }catch (Exception e) {
            Log.e(TAG, "Exception on bulkTransfer().");
        }
        ...
        if (mReadPakcetChecker) {
            ...
            if (updateReadCount) {
                Log.d(TAG, String.format(Locale.US, "Total Read Count : %d, Increment Read Count : %d.", totalReadCount, incReadCount));
